...
class Config:
    def __init__(self) -> None:
        # 是否延迟加载AI模型
        self.is_lazy_load_modules = distutils.util.strtobool(
            os.getenv('IS_LAZY_LOAD_MODULES') if os.getenv('IS_LAZY_LOAD_MODULES') else 'false')
        logger.warning('是否延迟加载AI模型: %s', ('延迟加载' if self.is_lazy_load_modules else '启动加载'))

        # 是否保存识别结果图片
        self.is_visualization = distutils.util.strtobool(
            os.getenv('IS_VISUALIZATION') if os.getenv('IS_VISUALIZATION') else 'false')
